 ...
 * Dijkstra不能为负权，因为算法中使用了优先队列和BFS思想，
 * 若有负权，则先出队列的可能并不是全局路径最短的，
 * 因为后面可能还有负值更大（即更短的路径），
 * 比如求(a, c)路径，有路径a -> b (-10)-> c(-10)，则为-20时，c出队列，
 * 而有另外路径a -> d(1) -> c(-100)，d的权为1因此出不了队列，
 * 后面(d,c)为-100的权就无法在c出队列之前更新c了，因此造成最短路径为(a,b,c)，实际上最短路径为(a,d,c)。
 */
