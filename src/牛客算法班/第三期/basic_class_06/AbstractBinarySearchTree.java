...
public class AbstractBinarySearchTree {
	...
	protected Node delete(Node deleteNode) {
		if (deleteNode != null) {
			...
			if (deleteNode.left == null) {
				...
				Node successorNode = getMinimum(deleteNode.right);// 找到后继节点，即右子树最小的节点
				if (successorNode.parent != deleteNode) {// 后继节点的父节点不是要删除的节点
	...
	private Node transplant(Node nodeToReplace, Node newNode) {
		if (nodeToReplace.parent == null) {
			this.root = newNode;// 替换根节点
		} else if (nodeToReplace == nodeToReplace.parent.left) {//如果是父节点的左孩子
		...
		if (newNode != null) {
			newNode.parent = nodeToReplace.parent;// 如果不为空，指向父节点
		}
		return newNode;
	}
